...
func (sm *SecurityManager) RestoreSystem() error {
	...
	if _, err := os.Stat(settingsFile); err == nil {
		if err := sm.enhancement.integrityChecker.VerifyIntegrity(settingsFile); err != nil {
			...
			// 監査ログに記録
			sm.enhancement.auditTrail.LogSecurityEvent("INTEGRITY_CHECK_FAILED", fmt.Sprintf("Settings file integrity check failed: %v", err))
		} else {
...
func (sm *SecurityManager) ValidateSecurityStatus() error {
	...
	// 認証設定の検証
	if err := sm.authManager.protector.ValidateConfig(); err != nil {
