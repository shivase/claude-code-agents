...
func NewMessageClient(sessionName string) *MessageClient {
	...
	// セッション名が空の場合は自動検出
	if sessionName == "" {
		if detectedSession, err := tmuxManager.FindDefaultAISession(); err == nil {
...
func (mc *MessageClient) GetStatus(agent string) (bool, error) {
	...
	// tmuxペインが存在するかチェック
	cmd := exec.Command("tmux", "list-panes", "-t", mc.sessionName)
	output, err := cmd.Output()
	if err != nil {
		return false, fmt.Errorf("failed to check pane status: %v", err)
	}
...
func (mc *MessageClient) CheckConnection() error {
	...
	if !mc.tmuxManager.SessionExists(mc.sessionName) {
		detectedSession, sessionType, err := mc.tmuxManager.DetectActiveAISession()
		if err != nil {
			return fmt.Errorf("no active AI sessions found: %v", err)
		}
		...
		if err != nil {
			return fmt.Errorf("failed to get pane count for session %s: %v", mc.sessionName, err)
		}
		...
		if paneCount != 6 && paneCount != 1 {
			detectedSession, sessionType, err := mc.tmuxManager.DetectActiveAISession()
			if err != nil {
				return fmt.Errorf("session %s has %d panes (not AI session) and no other AI sessions found: %v", mc.sessionName, paneCount, err)
			}
